...
function getImage(request, reply) {
   let file = Path.join(__dirname + "/" + request.params.name);

  // if (type == "thumbs") {
  //   return dbx
  //     .filesGetThumbnail({ path: "/" + request.params.name })
  //     .then(function(data) {
  //       Fs.writeFile("./temp.jpg", data.fileBinary, "binary", function(err) {
  //         if (err) {
  //           return reply(Boom.wrap(err, "Internal server error"));
  //         }
  //         return reply.file("./temp.jpg");
  //       });
  //     })
  //     .catch(function(err) {
  //       console.log(err);
  ...
  return dbx
    ...
    .then(function(data) {
      // Fs.writeFile(file, data.fileBinary, "binary", function(err) {
      //   if (err) {
      //     return reply(Boom.wrap(err, "Internal server error"));
      //   }
        reply.file(data.fileBinary).encoding('binary');
